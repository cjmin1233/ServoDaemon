...
struct ServoParams {
    ...
    uint16_t posCommandAvgFilter; // 0x210A
    ...
    uint16_t moveToZeroAfterHoming; // 0x201E
    ...
    uint32_t shaftRevolutions;  // 0x6091:02 -> Gear ratio = motor revolutions /
    // shaft revolutions
    uint32_t leadMm;   // Distance per motor revolution (mm)
    uint32_t strokeMm; // Maximum travel range (mm)
};

static constexpr uint32_t ppr[] = {
    0 /* Dummy */, 1 << 18, 1 << 19
}; // 0x2002: encoder pulse per revolution
// (LS mecapion: automatically configured)
static constexpr int slaveCountMax = 2;

// 기본값 정의는 servoconfig.cpp 에 있음 (ODR 방지)
// 런타임에 ConfigLoader::load() 를 통해 JSON 값으로 덮어쓸 수 있음
extern ServoParams SlaveConfigs[slaveCountMax + 1];
} // namespace ServoConfig
